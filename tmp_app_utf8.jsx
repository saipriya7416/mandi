...
const formatCurrency = (v) => "Γé╣" + (Number(v) || 0).toLocaleString("en-IN");
...
export default function App() {
  ...
  const handleRegisterSupplier = async () => {
    if (!supplierForm.name || !supplierForm.phone) return alert("ΓÜá∩╕Å Name and Phone are required");
    const res = await MandiService.addSupplier(supplierForm);
    if (res?.status === "SUCCESS") {
      alert("≡ƒÆ╛ SUCCESS: Supplier saved to MongoDB!");
  ...
  const handleOnboardBuyer = async () => {
    if (!buyerForm.name || !buyerForm.phone) return alert("ΓÜá∩╕Å Name and Phone are required");
    const res = await MandiService.addBuyer(buyerForm);
    if (res.status === "SUCCESS") {
      alert("≡ƒÆ╛ SUCCESS: Buyer details saved to MongoDB!");
  ...
  const handleCreateLot = async () => {
    if (!intakeForm.product || !intakeForm.quantity) return alert("ΓÜá∩╕Å Product and Qty are required");
    const res = await MandiService.addLot({
      ...intakeForm,
      supplier: intakeForm.supplierId || (suppliers[0]?._id) // Fallback to first if empty
    });
    if (res.status === "SUCCESS") {
      alert("≡ƒÆ╛ SUCCESS: Inventory Lot recorded in Database!");
  ...
  const ALL_MENU = [
    { id: "Dashboard", icon: "≡ƒôè", roles: ["Admin", "Accountant", "Operations Staff"] },
    { id: "User Roles", icon: "≡ƒæÑ", roles: ["Admin"] },
    { id: "Supplier Management", icon: "≡ƒÅó", roles: ["Admin", "Operations Staff"] },
    { id: "Buyer Management", icon: "≡ƒÆÄ", roles: ["Admin", "Operations Staff"] },
    { id: "Inventory Intake", icon: "≡ƒôÑ", roles: ["Admin", "Operations Staff"] },
    { id: "Inventory Allocation", icon: "≡ƒôñ", roles: ["Admin", "Operations Staff"] },
    { id: "Supplier Bill", icon: "≡ƒº╛", roles: ["Admin", "Accountant"] },
    { id: "Buyer Invoice", icon: "≡ƒôæ", roles: ["Admin", "Accountant"] },
    { id: "Ledger System", icon: "≡ƒôû", roles: ["Admin", "Accountant"] },
    { id: "Payment Management", icon: "≡ƒÆ│", roles: ["Admin", "Accountant"] },
    { id: "Expense Management", icon: "≡ƒÆ╕", roles: ["Admin", "Accountant", "Operations Staff"] },
    { id: "Verification & Compliance", icon: "≡ƒ¢í", roles: ["Admin"] },
    { id: "Reports", icon: "≡ƒôä", roles: ["Admin", "Accountant"] },
    { id: "Search & Filters", icon: "≡ƒöì", roles: ["Admin", "Accountant", "Operations Staff"] },
      ...
      {loggedIn && (
        <nav style={{ 
          ...
          <div style={{ marginTop: "auto", padding: "24px 24px 0", display: "flex", alignItems: "center", gap: "12px", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
             ...
             <button onClick={handleLogout} style={{ background: "none", border: "none", color: COLORS.accent, cursor: "pointer" }}>≡ƒÜ¬</button>
          </div>
        </nav>
      )}
        ...
        <div style={{ animation: "fadeIn 0.6s ease-out" }}>
          ...
          {activeSection === "Dashboard" && (
            <div style={{ display: "flex", flexDirection: "column", gap: "40px" }}>
              <div style={{ 
                ...
                {[
                  { label: "TOTAL REVENUE", val: "Γé╣2,84,560", trend: "+12.4%", trendLabel: "vs last week", color: COLORS.success },
              ...
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2fr 1.2fr", gap: "32px" }}>
                <Card title="≡ƒôê Revenue Trajectory">
                ...
                <Card title="≡ƒôª Category Weightage">
              ...
              {[
                ...
                <Card key={i} title={role.r} subtitle="Assigned Privileges">
                  <ul style={{ padding: 0, listStyle: "none" }}>
                    {role.f.map((feat, j) => <li key={j} style={{ padding: "12px 0", borderBottom: "1px solid #f1f5f9", fontWeight: "600", color: COLORS.text }}>ΓùÅ {feat}</li>)}
                  </ul>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 140px", gap: "32px" }}>
              <Card title="Supplier Directory" subtitle="Manage high-volume marketplace suppliers">
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "20px" }}>
                  <Button variant="secondary" onClick={() => MandiService.exportExcel('suppliers')}>≡ƒôè Export to Excel</Button>
                </div>
                ...
                <div style={{ display: "flex", gap: "12px" }}>
                  <Button onClick={handleRegisterSupplier} style={{ background: COLORS.success }}>≡ƒÆ╛ Save Supplier to Database</Button>
                </div>
                  ...
                  <tbody>
                    {suppliers.map(s => (
                      <tr key={s._id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                        ...
                        <td style={{ fontWeight: "900", color: COLORS.danger }}>Γé╣ {s.balance || 0}</td>
                        <td><Button variant="secondary" style={{ padding: "6px 14px" }}>≡ƒô£ History</Button></td>
                      </tr>
                    ))}
          ...
          {activeSection === "Buyer Management" && (
            <Card title="Buyer Ecosystem" subtitle="Customer profiles and credit tracking">
               <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "20px" }}>
                  <Button variant="secondary" onClick={() => MandiService.exportExcel('buyers')}>≡ƒôè Export to Excel</Button>
                </div>
              ...
              <div style={{ display: "flex", gap: "12px", marginBottom: "40px" }}>
                <Button onClick={handleOnboardBuyer} style={{ background: COLORS.success }}>≡ƒÆ╛ Save Buyer to Database</Button>
              </div>
          ...
          {activeSection === "Inventory Intake" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1.2fr", gap: "32px" }}>
              <Card title="≡ƒôÑ Intake Form">
                <Input label="≡ƒôà Entry Date" type="date" value={new Date().toISOString().slice(0, 10)} />
                <div style={{ marginBottom: "16px" }}>
                  <label style={{ display: "block", marginBottom: "8px", fontWeight: "800", color: COLORS.secondary, fontSize: "14px" }}>≡ƒÅó Select Supplier</label>
                  <select
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <Input label="≡ƒôª Product" placeholder="Mango / Apple" value={intakeForm.product} onChange={(e) => setIntakeForm({ ...intakeForm, product: e.target.value })} />
                  <Input label="≡ƒÅ╖ Grade" placeholder="A+ / Export" value={intakeForm.variety} onChange={(e) => setIntakeForm({ ...intakeForm, variety: e.target.value })} />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <Input label="ΓÜû∩╕Å Quantity" placeholder="e.g. 50" value={intakeForm.quantity} onChange={(e) => setIntakeForm({ ...intakeForm, quantity: e.target.value })} />
                  <select
                    style={{ height: "54px", width: "100%", background: "#f8fafc", borderRadius: "14px", border: "2.5px solid #f1f5f9", padding: "0 14px", fontWeight: "700" }}
                ...
                <Input label="≡ƒÆ░ Rate (Unit)" placeholder="Optional Market Rate" value={intakeForm.rate} onChange={(e) => setIntakeForm({ ...intakeForm, rate: e.target.value })} />
                ...
                <div style={{ padding: "20px", border: "2px dashed #e2e8f0", borderRadius: "16px", textAlign: "center", cursor: "pointer", marginBottom: "20px" }}>
                  ≡ƒô╖ Drop Produce or Bill Photos
                </div>
                <Button onClick={handleCreateLot} style={{ width: "100%" }}>Create Inventory Records</Button>
              </Card>
              <Card title="≡ƒôª Live Stock Intake" subtitle="Unallocated inventory awaiting buyer link">
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "20px" }}>
                  <Button variant="secondary" onClick={() => MandiService.exportExcel('inventory')} style={{ fontSize: "12px", padding: "8px 16px" }}>≡ƒôè Export Stock</Button>
                </div>
                {lots.map(lot => (
                  <div key={lot._id} style={{ padding: "20px", background: "#f8fafc", borderRadius: "16px", marginBottom: "16px", border: "1.5px solid #e2e8f0" }}>
                    ...
                    <p style={{ margin: "10px 0", fontSize: "14px" }}>Source: <b>{lot.supplier?.name}</b> | ΓÜû∩╕Å {lot.remaining} {lot.unit}</p>
          ...
          {activeSection === "Inventory Allocation" && (
            <Card title="≡ƒôñ Allocation Marketplace" subtitle="Track supplier-to-buyer distribution chain">
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "32px" }}>
                ...
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: "10px", alignItems: "end" }}>
                    ...
                    <Input label="Agreed Rate" placeholder="Γé╣" />
                    <Button onClick={() => alert("≡ƒôñ Allocation Mapping Sent to Matrix")} style={{ marginBottom: "20px" }}>Allocate</Button>
                  </div>
                  <table style={{ width: "100%", marginTop: "32px" }}>
                    ...
                    <tbody>
                      <tr><td>LOT-9921</td><td>Suri Traders</td><td>500 KG</td><td><span style={{ color: COLORS.success }}>ΓùÅ Mapped</span></td></tr>
                      <tr><td>LOT-9921</td><td>Kalyan Wholesale</td><td>750 KG</td><td><span style={{ color: COLORS.success }}>ΓùÅ Mapped</span></td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          {activeSection === "Supplier Bill" && (
            <Card title="≡ƒº╛ Digital Bill Composer" subtitle="Automatic calculations for physical mandi bills">
              ...
              <Card style={{ border: "2px solid #0f172a", marginBottom: "32px", padding: "40px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px", marginBottom: "32px" }}>
                  <div>≡ƒÑ¡ Mango</div><div>1250</div><div>Γé╣ 110</div><div>Γé╣ 1,37,500</div>
                </div>
              ...
              <div style={{ display: "flex", gap: "16px" }}>
                <Button onClick={() => alert("≡ƒÆ╛ Bill saved! Use backend IDs to test PDF download.")}>≡ƒÆ╛ Save Data</Button>
                <Button variant="success" onClick={() => MandiService.downloadBillPDF('TEST_ID')} style={{ opacity: 0.6 }}>≡ƒôä Download PDF Bill</Button>
                <Button variant="secondary">≡ƒû¿ Print</Button>
              </div>
            </Card>
          )}
          ...
          {activeSection === "Buyer Invoice" && (
            <Card title="≡ƒôæ Buyer Dispatch Invoice" subtitle="Commission-integrated sales billing">
              ...
              <div style={{ padding: "24px", background: "#f8fafc", borderRadius: "20px", marginBottom: "32px" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px" }}>
                  ...
                  <Input label="Transport" placeholder="Γé╣ 0.00" />
                  <Input label="Handling/Misc" placeholder="Γé╣ 0.00" />
                </div>
              </div>
                  ...
                  <tbody>
                    {[1, 2, 3].map(i => (
                      <tr key={i} style={{ background: "white", boxShadow: "0 2px 10px rgba(0,0,0,0.02)" }}>
                        ...
                        <td>1,250 KG</td>
                        <td style={{ color: COLORS.danger }}>- Γé╣ 45,000</td>
                        <td style={{ color: COLORS.success }}>+ Γé╣ 1,20,000</td>
          ...
          {activeSection === "Payment Management" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "32px" }}>
              <Card title="Incoming Collections (Buyers)">
                ...
                <Input label="Settlement Amount" placeholder="Γé╣ 0.00" type="number" />
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "12px", marginBottom: "20px" }}>
                  <Button variant="outline" style={{ padding: "12px", fontSize: "13px" }}>≡ƒÆ╡ Cash</Button>
                  <Button variant="outline" style={{ padding: "12px", fontSize: "13px" }}>≡ƒô▓ UPI / Scan</Button>
                  <Button variant="outline" style={{ padding: "12px", fontSize: "13px" }}>≡ƒÅª Bank Transfer</Button>
                </div>
              ...
              <Card title="Outgoing Disbursements (Suppliers)">
                ...
                <Input label="Transfer Amount" placeholder="Γé╣ 0.00" type="number" />
                <p style={{ color: COLORS.muted, fontSize: "12px" }}>Pending Net Payable: <b>Γé╣ 1,05,500</b></p>
            ...
            <Card title="Operational Burn Registry" subtitle="Track expenses per transaction or daily cycle">
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "16px", marginBottom: "40px" }}>
                {["Labour", "Transport", "Marketing", "Packing", "Misc"].map(cat => (
                  <div key={cat} style={{ padding: "24px", background: "#f8fafc", borderRadius: "20px", textAlign: "center", border: "1.5px solid #e2e8f0" }}>
                    ...
                    <h3 style={{ margin: "8px 0 0", color: COLORS.secondary }}>Γé╣ 24,000</h3>
                  </div>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: "32px" }}>
                <div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                    <Input label="Amount Paid" placeholder="Γé╣" />
          ...
          {activeSection === "Reports" && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "24px" }}>
              {[
                { t: "Supplier Transaction Report", i: "≡ƒÜÜ", d: "Deep dive into intake history & payouts" },
                { t: "Buyer Sales Report", i: "≡ƒ¢ì", d: "Analyze buyer volume & credit patterns" },
                { t: "Inventory Movement Report", i: "≡ƒôª", d: "Track stock flow from gate to dispatch" },
                { t: "Expense Audit Report", i: "≡ƒÆ╕", d: "Detailed category-wise burn analysis" },
                { t: "Financial Summary", i: "≡ƒÆ╣", d: "Consolidated P&L statement for owner" }
              ].map((rep, i) => (
