      ...
      <div style={{ flex: 1, padding: "30px" }}>
        ...
        <div style={{ background: "white", padding: "20px", borderRadius: "12px" }}>
          ...
          <div style={{ marginTop:"30px", background:"#f0fdfa", padding:"20px", borderRadius:"12px", border:"2px solid #14b8a6" }}>
            ...
            {buyers.map((b,index)=>(
              <div key={index} style={{ marginTop:"20px", border:"1px solid #14b8a6", borderRadius:"10px", padding:"10px" }}>
                {/* Buyer Info */}
                <div style={{ display:"flex", flexWrap:"wrap", gap:"10px" }}>
                  <input placeholder="Name" value={b.name} onChange={(e)=>updateBuyer(index,"name",e.target.value)} style={{ flex:"1", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="Phone" value={b.phone} onChange={(e)=>updateBuyer(index,"phone",e.target.value)} style={{ flex:"1", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="Shop Name" value={b.shopName} onChange={(e)=>updateBuyer(index,"shopName",e.target.value)} style={{ flex:"1", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="Address" value={b.address} onChange={(e)=>updateBuyer(index,"address",e.target.value)} style={{ flex:"2", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="Govt ID" value={b.govId} onChange={(e)=>updateBuyer(index,"govId",e.target.value)} style={{ flex:"1", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="ID Type" value={b.idType} onChange={(e)=>updateBuyer(index,"idType",e.target.value)} style={{ flex:"1", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="Credit Limit" value={b.creditLimit} onChange={(e)=>updateBuyer(index,"creditLimit",e.target.value)} style={{ flex:"1", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <input placeholder="Notes" value={b.notes} onChange={(e)=>updateBuyer(index,"notes",e.target.value)} style={{ flex:"2", padding:"8px", borderRadius:"6px", border:"1px solid #ccc" }} />
                  <button onClick={()=>deleteBuyer(index)} style={{ background:"#ef4444", color:"white", borderRadius:"6px", padding:"6px 10px" }}>❌</button>
                </div>
                ...
                <div style={{ marginTop:"15px" }}>
                  <div style={{ display:"flex", gap:"10px" }}>
                    {["Ledger","Purchase History","Outstanding","Payments"].map(tab=>{
                      const key = `${index}-${tab}`;
                      return <button key={key} onClick={()=>setActiveBuyerTab({...activeBuyerTab,[index]:tab})} style={{
                        padding:"6px 12px",
                        borderRadius:"6px",
                  ...
                  {/* Tab Content */}
                  <div style={{ marginTop:"10px", padding:"10px", border:"1px solid #0d9488", borderRadius:"8px", background:"#e0f2f1" }}>
                    {activeBuyerTab[index]==="Ledger" && <div>💼 Buyer Ledger Content</div>}
                    {activeBuyerTab[index]==="Purchase History" && <div>🛍 Purchase History Content</div>}
                    {activeBuyerTab[index]==="Outstanding" && <div>💳 Outstanding Tracking Content</div>}
                    {activeBuyerTab[index]==="Payments" && <div>💰 Payment Tracking Content</div>}
                  </div>
                </div>
