  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (storeName) {
        ...
        if (path === '/lot/allocate') {
          // 1. Auto-generate Buyer Invoice
          const invoices = getLocal('buyerInvoices');
          const inv = {
            _id: `auto_inv_${Date.now()}`,
            buyerId: body.buyerId,
            date: body.allocationDate,
            invoiceNo: body.buyerInvoiceNo || `AUTO-INV-${Date.now().toString().slice(-6)}`,
            items: [{ productName: body.lineItemId, quantity: body.quantity, rate: body.rate }],
            status: "Unpaid",
            autoGenerated: true,
            createdAt: new Date()
          };
          setLocal('buyerInvoices', [...invoices, inv]);

          // 2. Auto-generate/Update Supplier Bill (using Lot ID as partition)
          const bills = getLocal('supplierBills');
          const existingBill = bills.find(b => b.lotId === body.lotId);
          if (existingBill) {
            existingBill.items.push({ productName: body.lineItemId, quantity: body.quantity, rate: body.rate });
            existingBill.netPayable = (existingBill.netPayable || 0) + (body.quantity * body.rate);
            setLocal('supplierBills', bills.map(b => b._id === existingBill._id ? existingBill : b));
          } else {
            const bill = {
              _id: `auto_bill_${Date.now()}`,
              lotId: body.lotId,
              farmerId: body.farmerId || "sim_1", // Fallback if not provided
              date: body.allocationDate,
              billNo: `AUTO-BILL-${Date.now().toString().slice(-4)}`,
              items: [{ productName: body.lineItemId, quantity: body.quantity, rate: body.rate }],
              netPayable: body.quantity * body.rate,
              status: "Pending",
              autoGenerated: true,
              createdAt: new Date()
            };
            setLocal('supplierBills', [...bills, bill]);
          }
        }
