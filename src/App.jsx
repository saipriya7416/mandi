...
const getProductData = (productName) => {
  if (!productName) return { varieties: ["Standard", "Premium", "Local", "Hybrid"], grades: ["A Grade", "B Grade", "C Grade", "Premium", "Export Quality"], sizes: ["Small", "Medium", "Large", "Extra Large"], colors: ["Standard", "Mixed"] };
  ...
  return {
    ...
    colors: entry.colors || ["Standard", "Mixed", "Green", "Red", "Yellow", "Orange"]
  };
};
                  ...
                  <Card style={{ marginBottom: "24px", borderLeft: `6px solid ${COLORS.sidebar}` }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "20px" }}>
                       ...
                       <div>
                          ...
                          <select style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1.5px solid #F1F5F9", fontWeight: "700" }} value={intakeForm.buyerId} onChange={e => {
                             const b = buyers.find(x => (x.shopName || x.name) === e.target.value);
                             setIntakeForm({...intakeForm, buyerId: e.target.value, buyerMobile: b?.phone || b?.mobile || ""});
                          }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "20px", marginTop: "20px" }}>
                       <div>
                          <label style={{ fontSize: "11px", fontWeight: "800", color: COLORS.sidebar, textTransform: "uppercase", display: "block", marginBottom: "8px" }}>Buyer Mobile</label>
                          <input placeholder="+91 9999999999" style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1.5px solid #F1F5F9" }} value={intakeForm.buyerMobile} onChange={e => setIntakeForm({...intakeForm, buyerMobile: e.target.value})} />
                       </div>
                       <div>
                          <label style={{ fontSize: "11px", fontWeight: "800", color: COLORS.sidebar, textTransform: "uppercase", display: "block", marginBottom: "8px" }}>Destination Location</label>
                          <input placeholder="e.g. Madanapalle Market Hub" style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1.5px solid #F1F5F9" }} value={intakeForm.destination} onChange={e => setIntakeForm({...intakeForm, destination: e.target.value})} />
                       </div>
                       <div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "20px", marginTop: "20px" }}>
                       <div>
                          <label style={{ fontSize: "11px", fontWeight: "800", color: COLORS.sidebar, textTransform: "uppercase", display: "block", marginBottom: "8px" }}>Dispatch Status</label>
                          <select style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1.5px solid #F1F5F9" }} value={intakeForm.dispatchStatus} onChange={e => setIntakeForm({...intakeForm, dispatchStatus: e.target.value})}>
                             <option>Packed</option><option>Loaded</option><option>In Transit</option><option>Delivered</option>
                          </select>
                       </div>
                       <div>
                          <label style={{ fontSize: "11px", fontWeight: "800", color: COLORS.sidebar, textTransform: "uppercase", display: "block", marginBottom: "8px" }}>Payment Status</label>
                          <select style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1.5px solid #F1F5F9" }} value={intakeForm.paymentStatus} onChange={e => setIntakeForm({...intakeForm, paymentStatus: e.target.value})}>
                             <option>Pending</option><option>Partial</option><option>Paid</option>
                          </select>
                       </div>
                       <div>
                          <label style={{ fontSize: "11px", fontWeight: "800", color: COLORS.sidebar, textTransform: "uppercase", display: "block", marginBottom: "8px" }}>Payment Mode</label>
                          <select style={{ width: "100%", padding: "12px", borderRadius: "10px", border: "1.5px solid #F1F5F9" }} value={intakeForm.paymentMode} onChange={e => setIntakeForm({...intakeForm, paymentMode: e.target.value})}>
                  ...
                  {intakeForm.lineItems.map((item, index) => (
                    <Card key={item.itemGuid} style={{ marginTop: "16px", background: "#fcfcfc", border: index % 2 === 0 ? "1px solid #EBE9E1" : "1px solid #D4E157" }}>
                       ...
                       <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "14px", marginTop: "16px" }}>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Size</label>
                              <select style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.size} onChange={e => { const n=[...intakeForm.lineItems]; n[index].size=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }}>
                                 {getProductData(item.product).sizes.map(s => <option key={s}>{s}</option>)}
                              </select>
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Color</label>
                              <select style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.color} onChange={e => { const n=[...intakeForm.lineItems]; n[index].color=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }}>
                                 {getProductData(item.product).colors.map(c => <option key={c}>{c}</option>)}
                              </select>
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Quantity</label>
                              <input type="number" placeholder="0" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.grossWeight} onChange={e => { const n=[...intakeForm.lineItems]; n[index].grossWeight=Number(e.target.value); setIntakeForm({...intakeForm,lineItems:n}); }} />
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Unit Type</label>
                              <select style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.unit} onChange={e => { const n=[...intakeForm.lineItems]; n[index].unit=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }}>
                                 <option>KG</option><option>Box</option><option>Crate</option><option>Ton</option><option>Packet</option>
                              </select>
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Rate per KG (₹)</label>
                              <input type="number" placeholder="0.00" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.rate} onChange={e => { const n=[...intakeForm.lineItems]; n[index].rate=Number(e.target.value); setIntakeForm({...intakeForm,lineItems:n}); }} />
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Total Weight</label>
                              <input disabled style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "none", background: "#f0fdf4", fontWeight: "800", color: "#166534" }} value={`${item.grossWeight || 0} ${item.unit}`} />
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Total Amount (₹)</label>
                              <input disabled style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "none", background: "#f0fdf4", fontWeight: "900", color: "#166534" }} value={formatCurrency(item.rate * item.grossWeight)} />
                           </div>
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "14px", marginTop: "14px" }}>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>No. of Packages</label>
                              <input type="number" placeholder="0" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.packages} onChange={e => { const n=[...intakeForm.lineItems]; n[index].packages=Number(e.target.value); setIntakeForm({...intakeForm,lineItems:n}); }} />
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Packaging Type</label>
                              <select style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.packagingType} onChange={e => { const n=[...intakeForm.lineItems]; n[index].packagingType=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }}>
                                 <option>Plastic Crates</option><option>Wooden Boxes</option><option>Corrugated Box</option><option>Gunny Bags</option><option>Loose Load</option>
                              </select>
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Batch Number</label>
                              <input placeholder="e.g. BCH-2026-001" style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.batchNo} onChange={e => { const n=[...intakeForm.lineItems]; n[index].batchNo=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }} />
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Quality Status</label>
                              <select style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.qualityStatus} onChange={e => { const n=[...intakeForm.lineItems]; n[index].qualityStatus=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }}>
                                 <option>Passed</option><option>Quarantine</option><option>Rejected</option>
                              </select>
                           </div>
                           <div>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Storage Type</label>
                              <select style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.storageType} onChange={e => { const n=[...intakeForm.lineItems]; n[index].storageType=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }}>
                                 <option>Ambient</option><option>Cold Storage</option><option>Refrigerated</option><option>Hygienic Room</option>
                              </select>
                           </div>
                           <div style={{ gridColumn: "span 2" }}>
                              <label style={{ fontSize: "10px", fontWeight: "800", color: COLORS.muted, display: "block", marginBottom: "4px" }}>Dispatch Remarks</label>
                              <input placeholder="e.g. Handle with care, fragile, keep dry..." style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "1.2px solid #E2E8F0" }} value={item.remarks || ""} onChange={e => { const n=[...intakeForm.lineItems]; n[index].remarks=e.target.value; setIntakeForm({...intakeForm,lineItems:n}); }} />
                           </div>
                        </div>
                     </Card>
                   ))}
                   ...
                   <div style={{ marginTop: "20px", display: "flex", gap: "16px" }}>
                      ...
                      <Button variant="outline" style={{ background: "#fff", height: "60px" }} onClick={() => alert("Draft saved.")}>💾 Save Draft</Button>
                   </div>
